...
export const LiquidButtonClass = class LiquidButton {
  svg: SVGElement;
  svgText: any;
  id: number;
  tension: number;
  ...
  constructor(svg: SVGElement) {
    ...
    this.id =
      (this.constructor as any).id || ((this.constructor as any).id = 1);
  ...
  distance(p1: TouchesProps, p2: TouchesProps) {
      ...
      if (layerIndex === 1) {
        if (this.touches.length > 0) {
          ...
          for (
            ...
            const touch = this.touches[touchIndex];
            const gradient: any = document.createElementNS(
              this.xmlns,
              "radialGradient"
            );
