...
// void main(){
	...
	// vec2 mouse = uCoords * -0.5;
	
	// vec2 circlePos = st + mouse;
	// float c = circle(circlePos, 0.05, 2.0) * 2.5;

	// float offx = vUv.x + sin(vUv.y + uTime * .1);
	// float offy = vUv.y - uTime * 0.1 - cos(uTime * .001) * .01;

	// float n = snoise3(vec3(offx, offy, uTime * .01) * 7.) - 1.;

	// float finalMask = smoothstep(0.4, 0.5, n + pow(c, 2.));
	// vec4 image = texture2D(uTexture, vUv);

	// vec4 finalImage = mix(image, vec4(0.5,1.0,1.0,1.0), finalMask);
...
// }
	...
	// vec2 mouse = uCoords * -0.5;
    // //  vec2 st = vec2(gl_FragCoord.x, uRes.y - gl_FragCoord.y)/uRes;
	// vec2 mt = vec2((mouse.x) , uRes.y - (mouse.y))/uRes;