...
interface NavItemsPopupProps {
  ...
  styles: string;
  setShowNavigationItems: Dispatch<SetStateAction<boolean>>;
}
...
export const NavItemsPopup: React.FC<NavItemsPopupProps> = ({
  ...
  navigations,
  styles,
  setShowNavigationItems,
}) => {
  const popupRef = useRef<HTMLDivElement>(null);
  useOutsideClick(popupRef, { setState: setShowNavigationItems });

  return (
    <div
      ref={popupRef}
      className="absolute top-0 right-0 py-5 lg:px-14 px-6 bg-white text-black shadow flex flex-col space-y-7"
    >
